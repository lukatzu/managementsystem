...
class EmployeeGUI:
    ...
    def validate_and_add_employee(self):
        ...
        if not self.name_entry.get():
            self.name_entry.config(background="red")
            raise ValueError("Name cannot be empty")
        else:
            self.name_entry.config(background="white")

        if not self.dept_entry.get():
            self.dept_entry.config(background="red")
            raise ValueError("Department cannot be empty")
        else:
            self.dept_entry.config(background="white")
